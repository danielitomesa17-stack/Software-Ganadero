    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? 'w-72 p-6' : 'w-24 py-6 px-3'} bg-[#0F172A] flex flex-col transition-all duration-500 ease-in-out z-50 shadow-2xl h-screen`}>
        ...
        <nav className="flex-1 space-y-3 overflow-y-auto overflow-x-hidden custom-scrollbar pr-2 mb-4">
          <Link to="/app/inventario" className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 rounded-2xl transition-all ${isActive('/app/inventario') ? 'bg-green-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>
            <LayoutDashboard size={22} className="shrink-0" />
            {sidebarOpen && <span className="font-bold text-xs uppercase tracking-widest truncate">Inventario</span>}
          </Link>

          {/* MÓDULO ADMIN RESTRINGIDO */}
          {sesion.user?.rol === 'SuperAdmin' && (
            <Link to="/app/admin" className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 rounded-2xl transition-all ${isActive('/app/admin') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>
              <Settings size={22} className="shrink-0" />
              {sidebarOpen && <span className="font-bold text-xs uppercase tracking-widest truncate">Admin Panel</span>}
            </Link>
          )}

          <Link to="/app/reportes" className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 rounded-2xl transition-all ${isActive('/app/reportes') ? 'bg-rose-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>
          ...
          <Link to="/app/gastos" className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 rounded-2xl transition-all ${isActive('/app/gastos') ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800'}`}> 
            <Wallet size={22} className="shrink-0" />
            {sidebarOpen && <span className="font-bold text-xs uppercase tracking-widest truncate">Gastos</span>}
          </Link>

          <Link to="/app/ProduccionSistemas" className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 rounded-2xl transition-all ${isActive('/app/ProduccionSistemas') ? 'bg-green-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>
            <Scale size={22} className="shrink-0" />
            {sidebarOpen && <span className="font-bold text-xs uppercase tracking-widest truncate">Producción</span>}
          </Link>
          {/* Enlace a Sanidad */}
          <Link to="/app/SanidadSistemas" className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 rounded-2xl transition-all ${isActive('/app/SanidadSistemas') ? 'bg-amber-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>
            <Activity size={22} className="shrink-0" />
            {sidebarOpen && <span className="font-bold text-xs uppercase tracking-widest truncate">Sanidad</span>}
          </Link>
        </nav>

        <button onClick={onLogout} className={`flex items-center ${sidebarOpen ? 'gap-4 px-4' : 'justify-center'} py-4 text-slate-500 hover:text-red-400 mt-auto border-t border-slate-800 pt-6 transition-colors w-full shrink-0`}>
          <LogOut size={22} className="shrink-0" />
          {sidebarOpen && <span className="font-bold text-xs uppercase tracking-widest truncate">Cerrar Sesión</span>}
        </button>
      </aside>
